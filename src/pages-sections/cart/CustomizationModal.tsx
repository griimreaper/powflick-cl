        ...
        >
            ...
            <DialogContent dividers>
                ...
                {customization?.frontSide?.logos?.map((each, index) =>
                    each.logoUrl ? (
                        <Typography key={index} component="div">
                            Front Logo {index + 1}:{' '}
                            <img src={each.logoUrl} alt="Front Logo" style={{ width: 24, height: 24, marginLeft: 8 }} /> (+
                            $4.99)
                        </Typography>
                    ) : null
                )}
                ...
                {customization?.frontSide?.texts?.map((each, index) =>
                    each.text ? (
                        <Typography key={index}>Front Text {index + 1}: {each.text} (+ ${3.99})</Typography>
                    ) : null
                )}
                ...
                {customization?.backSide?.logos?.map((each, index) =>
                    each.logoUrl ? (
                        <Typography key={index} component="div">
                            Back Logo {index + 1}:{' '}
                            <img src={each.logoUrl} alt="Back Logo" style={{ width: 24, height: 24, marginLeft: 8 }} /> (+
                            $4.99)
                        </Typography>
                    ) : null
                )}
                ...
                {customization?.materials !== 'None' && customization && (
                    <Typography>
                        Materials: {customization.materials.split(' ')[1]
                            ? customization.materials.split(' ')[0] +
                            ' ($' +
                            Number(eliminarCaracteresNoNumericos(String(customization.materials.split(' ').pop()))) +
                            ')'
                ...
                <Box display="flex" justifyContent="center" gap={1}>
                    ...
                    {orderId === null && (
                        <Button
                            onClick={() => handleRemoveCustomization(productId, customization?.id!)}
                            variant="outlined"
                            color="error"
                            style={{ marginTop: 16 }}
                        >
